...
import { BsArrowUpRight, BsSortDown, BsSortUpAlt } from "react-icons/bs";
...
const Resume = () => {
  ...
  const fetchJobs = useCallback(async () => {
    ...
    let sanityQuery;
    if (filters.length > 0) {
      sanityQuery = encodeURIComponent(
        `*[_type == "job" && count((tags[])[@ in [${filters.map((filter) => `"${filter}"`)}]]) > 0] | order(startMonth ${sortType})`,
      );
    } else {
      sanityQuery = encodeURIComponent(
        `*[_type == "job"] | order(startMonth ${sortType})`,
      );
    }
  ...
  return (
    <PageScaffold extras="overflow-x-hidden">
      ...
      <div className="flex flex-col lg:flex-row pb-4 pt-4 gap-4">
        <ProjectCard
          name="JT Hair"
          description="Site for JT Hair of Brighton, MI"
          logoStyle={{ fontFamily: "JT", fontWeight: "lighter" }}
          icon={jt}
          date="March 2023"
        >
          <a href="https://jthaircare.com/" className="mt-2 sm:mt-0 btn-wire">
            Visit Site <BsArrowUpRight />
          </a>
        </ProjectCard>
        <ProjectCard
          name="Title Notes"
          description="Simple and stylish note-taking PWA"
          logoStyle={{ fontFamily: "TN", fontWeight: "500" }}
          icon={tn}
          date="October 2022"
        >
          <button
            className="mt-2 sm:mt-0 btn-wire"
            onClick={() => {
              window.scrollTo(0, 0);
              navigate("/title");
            }}
          >
            Try it out! <BsArrowUpRight />
          </button>
        </ProjectCard>
      </div>
      <div
        className="hero bg-transparent border-black dark:border-white rounded-3xl cursor-pointer mb-12"
        style={{ border: "1px solid" }}
      >
        <div
          className="hero-content flex-col items-start w-full"
      ...
      {loading ? (
        ...
        jobs.map((job, i) => {
          return (
            <JobEntry jobData={job} handleFilterPush={addFilter} key={i} />
          );
        })
      )}
    </PageScaffold>
